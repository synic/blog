...
---
<!-- summary -->
Rustaceans have been spoiled by Rust’s [clap](https://github.com/clap-rs/clap)
...
So I wrote [glap](https://github.com/synic/glap).
...
This is the part that felt the most like clap.
You describe your CLI as a struct, put some tags on the fields, and hand it to
glap:
...
You get `--help`, `--version`, validation of `possible` values, defaults,
required fields, short flags — all from the tags.

## The builder API

If you’d rather not use reflection (ya know, if you’re not into the whole
brevity thing), or you want to build commands dynamically at runtime, there’s a
builder API that does the same thing:
...
Same CLI, but without those pesky struct tags.
Pick whichever one fits your project.
...
It’s still pre-1.0 and the API might shift a bit before I tag a stable release,
but I’ve been using it in my own projects and it’s been solid.
The code, examples, and full docs live at
