...
I turned my nose up at initially, only to come around on them later. From cheese
...
even occur to me to try them any other way. It wasn’t until I was older that I
realized that when the skin starts getting brown spots on it, that’s when the
banana becomes sweet and delicious.

Another thing I hated when I was younger was 80s music. To me, at the time, it
sounded like a game show host trying to sing a song with the most game-showy
voice he could possibly muster. It’s all my girlfriend wanted to listen to and I
...
1. [Docker](https://www.docker.com/) - as I mentioned in the intro, I disliked
   ...
   had to redo my environment with Docker. For this reason, I avoided it for a
   ...
   what Docker was or how it worked at that point (that it shares the kernel
   with the host and these “virtual machines” are nothing more than regular
   Linux processes in a fancy shirt). Today, I don’t know what I would do
   without it.
2. [Tailwind](https://tailwindcss.com/) - this one is pretty obvious. I’m not
   the only one who hated it at first. It’s one of those things that is
   completely at odds with a lot of the practices and “rules” you’ve learned. It
   actively breaks rules in favor of simplicity and convenience. It’s exactly
   the opposite of how you learned to do CSS in the first place. In the end,
   though, it turns out that simplicity and convenience are well worth it.
3. [Go](https://go.dev/) - Oh, Go. My first experience with it was during a
   ...
   Can’t they even follow their own naming standards? Like Tailwind, it also
   takes practices and rules that we’ve been learning for years and years and
   turns them on their head. It actively questions the status quo and rejects
   ideologies that we’ve come to believe were required. Later on, when I was
   trying to figure out a solution for this blog, I wanted something that would
   still be relevant in several years. This blog is old, and it’s gone through
   ...
   require a lot of upkeep and package updating—so that if I didn’t end up
   writing articles for a while, the language wouldn’t change so much that I’d
   need to do a major overhaul every 6 months or so just to keep up with
   security patches. Go seemed like it checked all my boxes, so I set out to
   learn it. Once I knew why everything was the way it was (including the case
   mismatch in struct field names: names that start with a capital letter are
   public, and names that start with a lowercase letter are private outside the
   package), I began to really like it. At this point, I’d probably put it on
   the list of my favorite languages. It’s almost like you get C performance out
   of a language that feels a lot like writing Python or some other high-level
   scripting language. And the deployment story is unmatched; Go, in most cases,
   produces a binary that has no dependencies. You can even embed all your
   assets in the binary if you want.
...
really kick the tires. What would have changed if I’d had this realization from
the start?
